...
| 7 | `templates/starter/` とconsumer workflow | active | `adapters/web-jekyll-legacy/templates/` | Jekyll scaffold・Pages QAに特化 | scaffold、Book QA、consumer workflowのpath破壊 | #90/#94/#96。互換shimまたは新CLI経路と生成fixtureが必要 |
| 8 | top-level `templates/_config.yml`, `_data/`, `_includes/`, `assets/` | legacy / unknown | `archive/legacy-book-publishing-template-v3/template-snapshots/` またはconsumer確認後のadapter path | `_config.yml`のactiveな直接consumerは未確認で、他snapshotもstarter/sharedと差分がある。一部assetはlegacy文書から参照される | 外部の手動利用、legacy文書、consumer固有copy元を見落とす | #103でfile単位のconsumer/ownershipを監査し、activeだけadapterへ移管。未使用を証明したsnapshotだけ#102でarchive |
