...
| 4 | `docs/_includes/`, `docs/includes/`, `docs/assets/js/safe-main.js` | unknown | `archive/legacy-book-publishing-template-v3/component-snapshots/` | shared/starterと複製・差分があり、formatter自身のruntime入力とは確認できない | 実は手動copy元である可能性、外部文書からのpath参照 | #96で正本を固定し、hash/consumer監査後に#102。確定まで移動禁止 |
| 5 | root `TROUBLESHOOTING.md` とJekyll中心の `docs/*.md` | legacy | `adapters/web-jekyll-legacy/docs/` | Jekyll v3 / Pagesの運用知識であり、マルチチャネル共通文書と分離すべき | 現行consumerの唯一の手順を失う、相対リンク切れ | #96でadapter docsを成立させ、#103で新しい共通入口を作成してから移動 |
