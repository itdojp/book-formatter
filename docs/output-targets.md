...
`note-v1`はfree/paid Markdown・HTML fragment、有料ラインmanifest、手動checklist、検証済み画像/PDF候補を生成する。noteへの投稿、upload、販売設定は行わず、公開者が編集画面とログアウト表示を確認する。詳細は[`note` adapter contract](../adapters/note/README.md)を参照する。
