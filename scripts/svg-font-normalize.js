...
function rewriteSvgFonts(svgText) {
  ...
  // Repair a common invalid pattern generated by some tools/LLMs where a
  // double-quoted SVG attribute value contains unescaped double quotes.
  // Example (invalid XML):
  //   font-family="-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, ..."
  //
  // We normalize it to our canonical stack before running the general matchers.
  const brokenSansAttrRe =
    /font-family="-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif"/g;

  let text = svgText;
  const repaired = (text.match(brokenSansAttrRe) || []).length;
  if (repaired > 0) {
    text = text.replace(brokenSansAttrRe, `font-family="${SANS_STACK}"`);
    changed = true;
    changes += repaired;
  }
  ...
  const rewrittenCss = text.replace(cssRe, (full, prefix, value, suffix) => {
